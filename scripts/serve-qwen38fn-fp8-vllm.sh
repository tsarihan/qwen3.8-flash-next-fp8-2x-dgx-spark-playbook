#!/usr/bin/env bash
# Qwen3.8-Flash-Next-FP8 on 2x DGX Spark (GB10, sm_121), vLLM TP=2.
#   spark-1: NODE_RANK=0 ./serve-qwen38fn-fp8-vllm.sh
#   spark-2: NODE_RANK=1 ./serve-qwen38fn-fp8-vllm.sh
#
...
MODEL_DIR="${MODEL_DIR:-/data/models/qwen3.8-flash-next-fp8}"
IMAGE="${IMAGE:-vllm/vllm-openai:qwen38-flash-next}"
NAME="${NAME:-qwen38fn-fp8-vllm}"
PORT="${PORT:-8891}"
NODE_RANK="${NODE_RANK:?set NODE_RANK=0 on spark-1, 1 on spark-2}"
MASTER_ADDR="${MASTER_ADDR:-${NODE0_FABRIC}}"
...
case "$NODE_RANK" in
  0) HOST_IP=${NODE0_FABRIC} ;;
  1) HOST_IP=${NODE1_FABRIC} ;;
esac
...
docker run -d --name "$NAME" --entrypoint bash \
  ...
  -e NCCL_IB_ADDR_RANGE=${FABRIC_SUBNET} \
